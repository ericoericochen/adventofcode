...
def main(boxes: list[list[int]], m: int):
    N = len(boxes)
    ...
    for circuit in boxes_to_circuits.values():
        if not circuit.issubset(visited):
            circuits.append(circuit)
            visited = visited | circuit

    circuit_lengths = sorted([len(c) for c in circuits], reverse=True)
    result = compute_product(circuit_lengths[:3])
    return result


boxes = read_junction_boxes(input_file)
result = main(boxes, 1000)
# result = main(boxes, 10)
print("result: ", result)
